...
- **Auth:** After `user verify-email`, prefer long-lived **`api_key`** via `account api-key` (`GET /accounts/me`). Export **`POMA_API_TOKEN`** or pass **`--token`** / **`--json`**.
...
3. For dynamic path segments (IDs), use **`EncodePathSegment`** — never concatenate raw user strings into URLs.
...
4. **Validate inputs** before calling the client:
   ...
   - **`validate_safe_output_dir`** for any path written under user control (downloads)
   - **`reject_control_chars`** / **`validate_user_strings`** for text flags as appropriate
   - New flags used from **`--json`**: add fields to **`fileConfig`** in **`config.go`**, **`mergeConfigIntoFlags`**, and **`validate_file_config`** in **`safety.go`**.
...
| `internal/cli/safety.go` | Input validation helpers |
